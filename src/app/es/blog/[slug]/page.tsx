...
export async function generateMetadata({
  ...
  const ogImage = ogImageHref('es', slug);

  return {
    title: `${post.data.title} — Khriztian Moreno`,
    description,
    alternates: {
      canonical: postHref('es', slug),
      languages: {
        en: postHref('en', slug),
        es: postHref('es', slug),
      },
...
export default async function PostPageEs({ params }: PostPageProps) {
  const { slug: rawSlug } = await params;
  const slug = decodeRouteParam(rawSlug);
  const post = getPostBySlug('es', slug);
  ...
  return <PostDetail lang="es" post={post} />;
}
